...
router.get('/weeklysum', verifyJWT, async (req, res) => {
    ...
    const [year, week] = getWeekNumber(startOfWeek);
    ...
    try {
        ...
        const result = await pool.request()
            .input('user_id', sql.Int, user_id)
            .input('startOfWeek', sql.DateTime, startOfWeek) // Bind startOfWeek correctly
            .query(`
                WITH MoodCounts AS (
                    SELECT 
                    ...
                    WHERE 
                        ...
                        AND entry_date <= DATEADD(day, 6, @startOfWeek) -- Adjust for Sunday to Saturday
                    GROUP BY 
